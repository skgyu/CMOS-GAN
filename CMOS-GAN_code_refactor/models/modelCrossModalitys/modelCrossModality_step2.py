...
def corresponding_dataY_image(self, idx,bs):


    if idx<bs:

        return  self.data_Y_source[idx:idx+1], 1 if np.random.uniform(0, 1)<self.opt.use_data_probe else 0

    else:

        return torch.zeros( self.one_data_shape ).cuda(), 0 
...
def semi_hard_negative_mining(self,numpy_feature_fake_Y_source, numpy_feature_fake_Y_target,log=False):
    ...
    self.one_data_shape= torch.Size([1]+list(self.data_X_source.size()[1:]))
    ...
    source_positive_samples_dataY = []
    source_negative_samples_dataY = []
    # target_positive_samples_dataY = []
    target_negative_samples_dataY = []


    source_positive_samples_use_dataY = []
    source_negative_samples_use_dataY = []
    # target_positive_samples_use_dataY = []
    target_negative_samples_use_dataY = []
    ...
    for i,negative_pos in tqdm( enumerate(negative_poses) ):
        ...
        if i< bs:
            ...
            if sorted_classes[i][negative_pos]==classes[i]:
                ...
                if att_name=='source_imageXs':

                    negative_sample_dataY=  getattr(face_dataset,'source_imageYs')[ id2images[neg_class_id][random_image_num]   ]  
                    source_negative_samples_dataY.append( face_dataset.get_samples_from_loc(negative_sample_dataY).cuda() )
                    source_negative_samples_use_dataY.append(1 if np.random.uniform(0, 1)<self.opt.use_data_probe else 0 )

                else:

                    source_negative_samples_dataY.append(   torch.zeros( self.one_data_shape   ).cuda()    )
                    source_negative_samples_use_dataY.append(0)
                ...
                new_dataY,labelY=self.corresponding_dataY_image(sorted_indexs[i][negative_pos],bs)
                source_negative_samples_dataY.append(   new_dataY    )
                source_negative_samples_use_dataY.append(labelY)
                ########################
            ...
            if sorted_classes[i][positive_pos]!=classes[i] or sorted_indexs[i][positive_pos]==i:
                ...
                ########### 4.11
                positive_sample_dataY=  face_dataset.source_imageYs[ source_id2images[real_class][random_image_num]   ]  
                source_positive_samples_dataY.append(face_dataset.get_samples_from_loc(positive_sample_dataY).cuda() )
                source_positive_samples_use_dataY.append(1 if np.random.uniform(0, 1)<self.opt.use_data_probe else 0)
                ###########
                ...
                ########### 4.11
                new_dataY,labelY=self.corresponding_dataY_image(sorted_indexs[i][positive_pos],bs)
                source_positive_samples_dataY.append(   new_dataY    )
                source_positive_samples_use_dataY.append(labelY)
                ########################
            ...
            if sorted_classes[i][negative_pos]==classes[i]:
                ...
                if att_name=='source_imageXs':
                    ...
                    negative_sample_dataY=  getattr(face_dataset,'source_imageYs')[ id2images[neg_class_id][random_image_num]   ]  
                    target_negative_samples_dataY.append( face_dataset.get_samples_from_loc(negative_sample_dataY).cuda() )
                    target_negative_samples_use_dataY.append(1 if np.random.uniform(0, 1)<self.opt.use_data_probe else 0)

                else:

                    target_negative_samples_dataY.append(   torch.zeros( self.one_data_shape   ).cuda()    )
                    target_negative_samples_use_dataY.append(0)
                ...
                new_dataY,labelY=self.corresponding_dataY_image(sorted_indexs[i][negative_pos],bs)
                target_negative_samples_dataY.append(   new_dataY    )
                target_negative_samples_use_dataY.append(labelY)
                ########################
            ...
            # target_positive_samples_dataY.append(  torch.zeros( self.one_data_shape ).cuda() )
            # target_positive_samples_use_dataY.append(0)
            # ###########
    ...
    self.source_positive_samples_dataY = torch.cat(source_positive_samples_dataY,dim=0).cuda()
    self.source_negative_samples_dataY = torch.cat(source_negative_samples_dataY,dim=0).cuda()
    # self.target_positive_samples_dataY = torch.cat(target_positive_samples_dataY,dim=0).cuda()
    self.target_negative_samples_dataY = torch.cat(target_negative_samples_dataY,dim=0).cuda()  
    

    self.source_positive_samples_use_dataY = torch.Tensor(source_positive_samples_use_dataY).unsqueeze(1).unsqueeze(2).unsqueeze(3).cuda()
    self.source_negative_samples_use_dataY = torch.Tensor(source_negative_samples_use_dataY).unsqueeze(1).unsqueeze(2).unsqueeze(3).cuda()
    # self.target_positive_samples_use_dataY = torch.Tensor(target_positive_samples_use_dataY).unsqueeze(1).unsqueeze(2).unsqueeze(3).cuda()
    self.target_negative_samples_use_dataY = torch.Tensor(target_negative_samples_use_dataY).unsqueeze(1).unsqueeze(2).unsqueeze(3).cuda()    
...
def train_G(self):
    ...
    if 'hard_mining' in opt and opt.hard_mining and opt.lambda_triplet>0:
        ...
        with torch.no_grad():
            ...
            if opt.use_data_Tri:
                fake_Y_source_positive_samples=(1-self.source_positive_samples_use_dataY)*fake_Y_source_positive_samples \
                    +self.source_positive_samples_use_dataY*self.source_positive_samples_dataY            

                fake_Y_source_negative_samples=(1-self.source_negative_samples_use_dataY)*fake_Y_source_negative_samples \
                    +self.source_negative_samples_use_dataY*self.source_negative_samples_dataY
        ...
        with torch.no_grad():
            ...
            # fake_Y_target_positive_samples=(1-self.target_positive_samples_use_dataY)*fake_Y_target_positive_samples
            #     +self.target_positive_samples_use_dataY*self.target_positive_samples_dataY            

            if opt.use_data_Tri:
                fake_Y_target_negative_samples=(1-self.target_negative_samples_use_dataY)*fake_Y_target_negative_samples  \
                    +self.target_negative_samples_use_dataY*self.target_negative_samples_dataY
